...
def start_menu(message):
    ...
    bot.send_message(message.chat.id, description, reply_markup=keyb.add(
        types.InlineKeyboardButton("Выбрать группу", url="https://telegram.me/Testingframebot?startgroup=")),
                     parse_mode='Markdown')
...
def call_back(call):
    if call.data == "check":
        ...
        bot.send_message(call.message.chat.id, f'Не верно.\nОтвет: *{QA[chats[call.message.chat.id]["num"]]["right"]}*',
                         parse_mode='Markdown')
...
def initilaze_dict(id):
    ...
    bot.send_message(id, f'*{QA[randomize]["caption"]}*', parse_mode='Markdown')
    keyb = types.InlineKeyboardMarkup()
...
def check_answer(message):
    ...
    elif chats[message.chat.id][message.from_user.username] == 15:
